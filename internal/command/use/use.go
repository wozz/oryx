...
func execute(c *cli.Context) error {
	...
	arg1 := c.Args().First()
	if arg1 == "" {
		return fmt.Errorf("usage: oryx use [OWNER/]REPO TAG (example: oryx use rules_go 0.18.3)")
	}
	...
	if len(parts) == 1 {
		// case like 'oryx use rules_go'
		owner = "bazelbuild"
		repo = parts[0]
	} else if len(parts) == 2 && len(parts[1]) == 40 {
		// case like 'oryx use rules_go ef7cca8857f2f3a905b86c264737d0043c6a766e'
		owner = "bazelbuild"
		repo = parts[0]
	} else if len(parts) == 2 {
		// case like 'oryx use bazelbuild/rules_go ef7cca8857f2f3a905b86c264737d0043c6a766e'
		owner = parts[0]
		repo = parts[1]
	} else {
	...
	err = Download(resp.Body, tmpFile, resp.ContentLength, url)
	if err != nil {
	...
	//
	sha256, err := GetFileSha256(tmpFile.Name())
	if err != nil {
